...
exports.getLatestROMS = ->
	...
	for region in ["ca", "cc", "ccc", "mb", "sfb", "nc1", "nc2"]
		for variable in ["curr", "salinity", "ssh", "temp"]
			...
			until now.getUTCFullYear() is 2012 or fs.existsSync "sampledata/#{now.getUTCFullYear()}/#{now.getUTCMonth() + 1}/#{region}_#{variable}#{padTo2Digits now.getUTCMonth() + 1}#{padTo2Digits now.getUTCDate()}_#{padTo2Digits now.getUTCHours()}_0.jpg"
